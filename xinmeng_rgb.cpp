...
static const KnownDevice KNOWN_DEVICES[] = {
    ...
    {0x0C45, 0x8006, "Xinmeng M87 Pro v2 (Wired mode)"},
};
...
static std::vector<DeviceInfo> find_known_devices() {
    ...
    for (size_t k = 0; k < KNOWN_DEVICES_COUNT; k++) {
        ...
        // Prefer vendor-specific usage page (0xFF00)
...
public:
    ...
    // hidapi hid_write() expects the first byte to be the Report ID.
...
static int cmd_replay(const std::string& json_path,
    ...
    // Minimal JSON parser: extract "hex" fields
    ...
    while (std::getline(f, line)) {
        ...
        auto q1 = line.find('"', hex_pos + 6);
        if (q1 == std::string::npos) continue;
        auto q2 = line.find('"', q1 + 1);
        if (q2 == std::string::npos) continue;
        std::string hex = line.substr(q1 + 1, q2 - q1 - 1);

        // Label filter: look in nearby context (simple approach: check within 5 lines)
        // For simplicity, we parse "label" in the same JSON object window.
        // We skip the filter here and just replay all for simplicity.
        (void)label_filter;
